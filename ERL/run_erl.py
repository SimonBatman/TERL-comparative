...
run_name = f"{env_tag}__{seed}__{int(time.time())}"
...
class Agent:
    ...
    def train(self):
        ...
        writer.add_scalar("charts/best_fitness", best_train_fitness, self.num_frames)
        ...
        elite_index, _, _ = self.evolver.epoch(self.pop, all_fitness)


        ####################### DDPG #########################
        #DDPG Experience Collection
        rl_actor_total_reward = self.evaluate(self.rl_agent.actor, is_render=False, is_action_noise=True) #Train
        print("rl_actor_total_reward", rl_actor_total_reward)
        writer.add_scalar("charts/rl_return", rl_actor_total_reward, self.num_frames)
